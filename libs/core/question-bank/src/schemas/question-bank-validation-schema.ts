...
import { z } from "zod";
import { questionTemplateSchema } from "./question-template-schema";
import { docSchema } from "./doc-schema";
import { annexSchema } from "./annex-schema";
import {  LearningObjectiveSchema } from "./learning-objective-schema";
import { subjectSchema } from "./subject-schema";
import { courseSchema } from "./course-schema";

export const questionBankValidation = z.object({
  ...
  const questionIds = makeMap(val.questionTemplates, (q) => q.id, () => true);
  const docIds = makeMap(val.docs, (d) => d.id, () => true);
  const annexIds = makeMap(val.annexes, (a) => a.id, () => true);
  const loIds = makeMap(val.learningObjectives, (l) => l.id, () => true);
  const subjectIds = makeMap(val.subjects, (s) => s.id, () => true);
  const courseIds  = makeMap(val.courses, (c) => c.id, () => true);
