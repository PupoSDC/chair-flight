...
type Data = AppRouterOutput["containers"]["questions"]["getQuestionEditorDiffTool"];
...
const SearchListItem = memo<{
  ...
  const recoverQuestion = useQuestionEditor((s) => s.undoMarkQuestionAsDeleted);
    ...
    const initial = {
      preview: getQuestionPreview(initialState),
    ...
    const current = currentState ?  {
      preview: getQuestionPreview(currentState),
    ...
    <ListItemContent sx={{ display: "flex" }}>
      ...
      >
        ...
        <Divider sx={{ my: 1, mx: 2 }} />
        <Typography sx={{ fontSize: "xs", color: hasRelatedQsChanged ? "warning.plainColor" : undefined }}>
          <b>Related Questions</b><br />
          {current.relatedQs}
        </Typography>
        <Typography sx={{ fontSize: "xs", color: hasLosChanged ? "warning.plainColor" : undefined }}>
          <b>Learning Objectives</b><br />
          {current.los}
        </Typography>
        <Typography sx={{ fontSize: "xs", color: hasAnnexesChanged ? "warning.plainColor" : undefined }}>
          <b>Annexes</b><br />
          {current.annexes}
        </Typography>
      </Stack>{/** */}
