import type { AppRouterOutput } from "@chair-flight/trpc/server";

export const questionBankGetAllSubjectsMock: AppRouterOutput["questionBank"]["getAllSubjects"] =
  {
    subjects: [
      {
        id: "010",
        courses: [],
        longName: "Air Law",
        shortName: "ALaw",
        numberOfExamQuestions: 44,
        numberOfExamMinutes: 60,
        learningObjectives: [
          "010.01",
          "010.02",
          "010.04",
          "010.05",
          "010.06",
          "010.07",
          "010.08",
          "010.09",
          "010.10",
          "010.11",
          "010.12",
          "010.13",
        ],
        numberOfQuestions: 1941,
      },
      {
        id: "021",
        courses: [],
        longName:
          "Aircraft General Knowledge — Airframe, Systems And Power Plant",
        shortName: "AGK",
        numberOfExamQuestions: 80,
        numberOfExamMinutes: 120,
        learningObjectives: [
          "021.01",
          "021.02",
          "021.03",
          "021.04",
          "021.05",
          "021.06",
          "021.07",
          "021.08",
          "021.09",
          "021.10",
          "021.11",
          "021.12",
          "021.13",
          "021.14",
          "021.15",
          "021.16",
          "021.17",
        ],
        numberOfQuestions: 1612,
      },
      {
        id: "022",
        courses: [],
        longName: "Aircraft General Knowledge - Instrumentation",
        shortName: "Instruments",
        numberOfExamQuestions: 60,
        numberOfExamMinutes: 90,
        learningObjectives: [
          "022.01",
          "022.02",
          "022.03",
          "022.04",
          "022.05",
          "022.06",
          "022.07",
          "022.08",
          "022.09",
          "022.10",
          "022.11",
          "022.12",
          "022.13",
          "022.14",
          "022.15",
        ],
        numberOfQuestions: 2649,
      },
      {
        id: "031",
        courses: [],
        longName: "Mass And Balance - Aeroplanes/Helicopters",
        shortName: "M&B",
        numberOfExamQuestions: 25,
        numberOfExamMinutes: 75,
        learningObjectives: ["031.01", "031.02", "031.04", "031.05", "031.06"],
        numberOfQuestions: 872,
      },
      {
        id: "032",
        courses: [],
        longName: "Performance - Aeroplanes",
        shortName: "Perf",
        numberOfExamQuestions: 45,
        numberOfExamMinutes: 120,
        learningObjectives: ["032.01", "032.02", "032.03", "032.04", "032.05"],
        numberOfQuestions: 1907,
      },
      {
        id: "033",
        courses: [],
        longName: "Flight Planning And Monitoring",
        shortName: "FPM",
        numberOfExamQuestions: 42,
        numberOfExamMinutes: 120,
        learningObjectives: [
          "033.01",
          "033.02",
          "033.03",
          "033.04",
          "033.05",
          "033.06",
        ],
        numberOfQuestions: 2357,
      },
      {
        id: "034",
        courses: [],
        longName: "Performance - Helicopters",
        shortName: "Perf(H)",
        learningObjectives: ["034.01", "034.02", "034.03", "034.04"],
        numberOfQuestions: 0,
        numberOfExamQuestions: 42,
        numberOfExamMinutes: 120,
      },
      {
        id: "040",
        courses: [],
        longName: "Human Performance And Limitations",
        shortName: "HPL",
        numberOfExamQuestions: 48,
        numberOfExamMinutes: 90,
        learningObjectives: ["040.01", "040.02", "040.03"],
        numberOfQuestions: 2189,
      },
      {
        id: "050",
        courses: [],
        longName: "Meteorology ",
        shortName: "Met",
        numberOfExamQuestions: 84,
        numberOfExamMinutes: 120,
        learningObjectives: [
          "050.01",
          "050.02",
          "050.03",
          "050.04",
          "050.05",
          "050.06",
          "050.07",
          "050.08",
          "050.09",
          "050.10",
        ],
        numberOfQuestions: 3395,
      },
      {
        id: "061",
        courses: [],
        longName: "General Navigation",
        shortName: "GNav",
        numberOfExamQuestions: 55,
        numberOfExamMinutes: 135,
        learningObjectives: ["061.01", "061.02", "061.03", "061.04", "061.05"],
        numberOfQuestions: 1692,
      },
      {
        id: "062",
        courses: [],
        longName: "Radio Navigation",
        shortName: "RNav",
        numberOfExamQuestions: 66,
        numberOfExamMinutes: 90,
        learningObjectives: ["062.01", "062.02", "062.03", "062.06", "062.07"],
        numberOfQuestions: 370,
      },
      {
        id: "070",
        courses: [],
        longName: "Operational Procedures",
        shortName: "OPS",
        numberOfExamQuestions: 42,
        numberOfExamMinutes: 75,
        learningObjectives: ["071.01", "071.02", "071.03", "071.04"],
        numberOfQuestions: 1939,
      },
      {
        id: "081",
        courses: [],
        longName: "Principles Of Flight - Aeroplanes",
        shortName: "POF",
        numberOfExamQuestions: 46,
        numberOfExamMinutes: 90,
        learningObjectives: [
          "081.01",
          "081.02",
          "081.03",
          "081.04",
          "081.05",
          "081.06",
          "081.07",
          "081.08",
        ],
        numberOfQuestions: 2640,
      },
      {
        id: "082",
        courses: [],
        longName: "Principles Of Flight - Helicopters",
        shortName: "POF(H)",
        numberOfExamQuestions: 46,
        numberOfExamMinutes: 90,
        learningObjectives: [
          "082.01",
          "082.02",
          "082.03",
          "082.04",
          "082.05",
          "082.06",
          "082.07",
          "082.08",
        ],
        numberOfQuestions: 0,
      },
      {
        id: "090",
        courses: [],
        longName: "Communications",
        shortName: "Coms",
        numberOfExamQuestions: 34,
        numberOfExamMinutes: 45,
        learningObjectives: [
          "090.01",
          "090.02",
          "090.03",
          "090.04",
          "090.05",
          "090.06",
          "090.07",
        ],
        numberOfQuestions: 1298,
      },
    ],
  };
