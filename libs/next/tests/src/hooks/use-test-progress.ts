...
import { userPreferences } from "@cf/next-old/user";
import type { Test, TestId } from "@cf/core/tests";

type TestProgress = {
  tests: Record<string, Test>;
  getTest: (args: { testId: TestId }) => Test;
  getCurrentQuestion: (args: { testId: TestId }) => Test["questions"][number];
  addTest: (args: { test: Test }) => void;
  startTest: (args: { testId: TestId }) => void;
  answerTestQuestion: (args: {
    testId: string;
    questionId: string;
    optionId: string;
  }) => void;
  goToTestQuestion: (args: { testId: TestId; questionId: string }) => void;
  goToNextQuestion: (args: { testId: TestId }) => void;
  goToPreviousQuestion: (args: { testId: string }) => void;
  tickTestTimer: (args: { testId: string; timeSpentInMs: number }) => void;
  finishTest: (args: { testId: TestId }) => void;
  deleteTest: (args: { testId: TestId }) => void;
};
...
export const useTestProgress = create<TestProgress>()(
  devtools(
    persist(
      ...
      { name: "cf-test-progress", version: 2 },
    ),
  ),
);
